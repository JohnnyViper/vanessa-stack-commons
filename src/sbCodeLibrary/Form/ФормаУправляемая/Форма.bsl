...
&НаСервере
Процедура ВКонфигурацииЕстьОбработкаСИменем(Имя)
	ИскомаяОбработка = Метаданные.Обработки.Найти(Имя);
	Если ИскомаяОбработка = Неопределено Тогда
		ВызватьИсключение "Обработка с именем """ + Имя + """ не найдена в метаданных";
	КонецЕсли;
КонецПроцедуры
